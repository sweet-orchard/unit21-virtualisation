                  ...
                  <div className="mb-10">
                    ...
                    <div className="space-y-6 mb-6">
                      ...
                      >
                        ...
                        <p className="text-slate-300 text-sm leading-relaxed mb-4">
                          Processing happens inside centralised servers, and gathers all the data, and users can connect to them remotely over network using a client device (thin client, zero client, or regular PC). The user’s device only displays the screen and sends keyboard /mouse input. The server does all the heavy computing (CPU, RAM, storage). Users can access their virtual desktop from anywhere with internet and they don’t powerful local hardware. However, it requires network connection to be able to connect to the remote VMs.
                        </p>
                        
                        <div className="grid md:grid-cols-3 gap-4">
                          {[
                            { type: 'Fat Client', icon: Monitor, char: 'It’s most expensive client device as it’s like a regular PC with its own operating system and some local processing power. It can run some applications locally but also connects to remote virtual desktops.', resources: 'High', color: 'sky' },
                            { type: 'Thin Client', icon: Layers, char: 'This is a lightweight device with minimal local OS and resources. It’s designed specifically to connect to remote desktops and it’s cheaper than a fat client, as it consumes less power.', resources: 'Low', color: 'amber' },
                    ...
                    >
                      <div className="space-y-4">
                        {[
                          ...
                          {
                            title: "Virtual Desktop Infrastructure (VDI) and Thin Client",
                            desc: "GamesTek can set up a centralised place for servers that run type 1 hypervisors to host all testing virtual machines. Instead of giving each tester a powerful PC, employees can use thin clients (simple devices that connect to the servers over the network). All the testing processes happen on the central servers, while testers' devices only display the screen and send keyboard and mouse input. When the team receives a new game build, it is installed on the server and everyone tests identically. This reduces the risk that a game works for one tester but not another. Test reports and documentation are stored on the server and can be shared with Top-game-shack during video calls. Benefits include cost savings, remote work capability, and all files stored in one organised place. However, it requires reliable network infrastructure because if the internet fails, all testing stops, and a backup server is needed in case the primary server fails.",
                          },
                          {
                  ...
                  {[
                    ...
                    { 
                      ...
                      items: [
                        ...
                        { title: 'Learning & Adaptation Period', desc: 'New features and a new way to test the games might be frustrating for testers, because some of them might not know about it. How to connect to VMs, using thin clients, managing snapshots - there are many things to learn for the employees and it might temporarily drop productivity while learning the new system.', positive: false },
                      ]
                    },
                  ].map((section, si) => (
                    <motion.div 
